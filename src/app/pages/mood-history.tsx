...
export function MoodHistory() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      const stats = allEntries.reduce((acc: Record<Mood, number>, entry: JournalEntry) => {
                ...
                <div className="mt-12 grid grid-cols-3 gap-6">
                  ...
                  >
                    <div className="text-3xl mb-2">
                      {Object.entries(moodStats).reduce((max, [mood, count]) => 
                        count > max.count ? { mood, count } : max
                      , { mood: 'happy', count: 0 }).count > 0
                        ? moodConfig[Object.entries(moodStats).reduce((max, [mood, count]) => 
                            count > max.count ? { mood: mood as Mood, count } : max
                          , { mood: 'happy' as Mood, count: 0 }).mood].emoji
                        : '—'
                      }
                    </div>
                    <div className="text-sm opacity-60" style={{ color: '#4A3F35' }}>
